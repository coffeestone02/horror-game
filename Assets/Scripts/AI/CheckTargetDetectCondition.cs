...
[Condition(name: "CheckTargetDetect", story: "Compare values of [CurrentDistance] and [ChaseDistance] ,And Is target in [Self] insight( [ViewHeight] )",
...
public partial class CheckTargetDetectCondition : Condition
{
    ...
    [SerializeReference] public BlackboardVariable<float> ViewHeight;
    public override bool IsTrue()
    {
        ...
        for (int i = 0; i < targets.Length; i++)
        {
            ...
            float angle = Vector3.Angle(Self.Value.transform.forward + Self.Value.transform.up * ViewHeight.Value, direction); // AI와 타겟 사이의 각도를 계산
            Debug.DrawRay(Self.Value.transform.position + Self.Value.transform.up * ViewHeight.Value, direction, Color.red);
            if (angle <= 90f * 0.5f) // 시야각 범위 내에 있는지 확인
            {
                Debug.Log("플레이어 포착");
                return true;
            }
